...
isCreditCardNumber(4916123456789012);
...
function getCommonDirectoryPath(pathes) {
  ...
  const splitP = pathes.map((path) => path.split('/'));
  const comP = splitP.reduce((com, path) => com.filter((dir, i) => dir === path[i]));
  return comP.length > 0 ? `${comP.join('/')}/` : '';
}
...
function getMatrixProduct(m1, m2) {
  ...
  m1.forEach((row, i) => {
    m2[0].forEach((_, j) => {
      result[i][j] = row.reduce((sum, val, k) => sum + val * m2[k][j], 0);
    });
  });
  return result;
}
